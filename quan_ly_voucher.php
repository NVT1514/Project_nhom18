...
if (isset($_GET['search']) && !empty(trim($_GET['search']))) {
    ...
    // Thêm điều kiện tìm kiếm vào mệnh đề WHERE: tìm kiếm theo ma_voucher HOẶC mo_ta
    $where_clause = " WHERE ma_voucher LIKE '%" . $conn->real_escape_string($search_term) . "%' 
                      OR mo_ta LIKE '%" . $conn->real_escape_string($search_term) . "%'";
}

// --- Thêm voucher (Giữ nguyên) ---
...
// --- Xóa voucher (Giữ nguyên) ---
if (isset($_GET['delete'])) {
    $id = intval($_GET['delete']);
    // Bổ sung thêm search_term để sau khi xóa vẫn giữ nguyên trạng thái tìm kiếm
    $redirect_url = "quan_ly_voucher.php?msg=deleted" . (!empty($search_term) ? "&search=" . urlencode($search_term) : "");
...
if (isset($_POST['edit_voucher'])) {
    ...
    // Bổ sung thêm search_term để sau khi sửa vẫn giữ nguyên trạng thái tìm kiếm
    $redirect_url_success = "quan_ly_voucher.php?msg=updated" . (!empty($search_term) ? "&search=" . urlencode($search_term) : "");
    $redirect_url_error = "quan_ly_voucher.php?msg=date_error&edit_id=" . $id . (!empty($search_term) ? "&search=" . urlencode($search_term) : "");
...
// --- Truy vấn dữ liệu với điều kiện tìm kiếm (MỚI) ---
// Thêm mệnh đề WHERE nếu có tìm kiếm
$query = "SELECT * FROM vouchers" . $where_clause . " ORDER BY id DESC";
...
<head>
    ...
    <style>
        ...
        /* MỚI: Định dạng cho thanh tìm kiếm */
        .search-container {
            width: 90%;
            max-width: 1100px;
...
<body>
    ...
    <div class="main-content">
        ...
        <form method="GET" class="search-container">
            <input type="text" name="search" class="form-control" placeholder="Tìm kiếm theo Mã Voucher hoặc Mô tả..."
                value="<?= htmlspecialchars($search_term) ?>">
            <button type="submit" class="btn btn-info">🔍 Tìm kiếm</button>
            <?php if (!empty($search_term)): // Hiện nút reset nếu đang tìm kiếm 
            ?>
                <a href="quan_ly_voucher.php" class="btn btn-secondary">❌ Bỏ tìm kiếm</a>
            <?php endif; ?>
        </form>

        <div class="d-flex justify-content-between mb-3">
            <h5>Danh sách voucher <?= !empty($search_term) ? '(Kết quả tìm kiếm)' : '' ?></h5>
                    ...
                    ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="?delete=<?= $row['id'] . (!empty($search_term) ? "&search=" . urlencode($search_term) : "") ?>" class="btn btn-sm btn-delete" onclick="return confirm('Xóa voucher này?')">Xóa</a>
                                </td>
                            </tr>
                            ...
                            <div class="modal fade" id="editModal<?= $row['id'] ?>" tabindex="-1">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form method="POST" action="quan_ly_voucher.php<?= !empty($search_term) ? "?search=" . urlencode($search_term) : "" ?>">
                        ...
                        <?php
                        endwhile;
                    else: // MỚI: Xử lý không có kết quả
                        ?>
                        <tr>
                            <td colspan="10" class="text-center">
                                😥 Không tìm thấy voucher nào <?= !empty($search_term) ? "với từ khóa **" . htmlspecialchars($search_term) . "**." : "" ?>
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
